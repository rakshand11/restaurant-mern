...
export const placeOrder = async (req, res) => {
    try {
        ...
        const totalAmount = cart.items.reduce((sum, item) => sum + item.menuItem.price * item.quantity, 0); //cart.items = returning the array ,reduce()=array method returns single value,calculating total cart amount by sum(p*q) for each item
...
export const getOrder = async (req, res) => {
    try {
        const { id } = req.user;
        const order = await orderModel.findOne({ user: id }).sort({ createdAt: -1 });
        res.status(200).json({
            msg: order
        });
        return;
